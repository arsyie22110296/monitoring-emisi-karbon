...
def plot_cluster_scatter(df: pd.DataFrame):
    ...
    for cluster_label in df['cluster_label'].unique():
        ...
        fig.add_trace(go.Scatter(
            ...
            text=df_cluster['tanggal'].dt.strftime('%Y-%m-%d') if 'tanggal' in df_cluster.columns else df_cluster.index,
...
def plot_weekly_pattern(df: pd.DataFrame):
    ...
    if 'tanggal' not in df.columns or 'total_emisi_kgco2' not in df.columns:
        fig = go.Figure()
